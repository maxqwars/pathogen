...
import { DatabaseTypes, TITLE_CONTENT_TYPE_CODE_ENUM, TITLE_SEASON_CODE_ENUM } from '@maxqwars/xconn'
...
const ReleaseDetails = (props: IReleaseDetailsProps) => {
  ...
  return (
    <div className={styles['release-details']}>
      <IonCard>
        <IonCardHeader>
          <IonCardTitle>{lng === 'ru' ? release?.names?.ru : release?.names?.en}</IonCardTitle>
        </IonCardHeader>
      </IonCard>

      <div className={styles['release-details__stats-box']}>
        <div className={styles['release-details__stat-item']}>
          <div className={styles['release-details__stat-item-value']}>{millify(release.inFavorites as number)}</div>
          <div className={styles['release-details__stat_item-sign']}>{t('saves-label').toLowerCase()}</div>
        </div>
        ...
        <div className={styles['release-details__stat-item']}>
          ...
          <div className={styles['release-details__stat_item-sign']}>{t('type-label').toLowerCase()}</div>
        </div>

        <div className={styles['release-details__stat-item']}>
          <div className={styles['release-details__stat-item-value']}>{release?.type?.series}</div>
          <div className={styles['release-details__stat_item-sign']}>{t('episodes-label').toLowerCase()}</div>
        </div>

        <div className={styles['release-details__stat-item']}>
          <div className={styles['release-details__stat-item-value']}>{release?.season?.year}</div>
          <div className={styles['release-details__stat_item-sign']}>{t('year-label').toLowerCase()}</div>
        </div>
        ...
        <div className={styles['release-details__stat-item']}>
          ...
          <div className={styles['release-details__stat_item-sign']}>{t('season-label').toLowerCase()}</div>
        </div>
      </div>
